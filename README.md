...
- **v1.3.2 (Released):** Guild Supply Chain, Two-Sided Bounties & Goal Management Suite (Two-Sided delivery verification handshake, live bag inventory counter, insufficient inventory safeguards, interactive Goals ➔ Bounties linking with golden card glow, top-5 HUD clamping, self-claiming support, and 3-line card layout overhaul).
- **v1.3.3 (Released):** Recipe Drops Revamp, Robust Profession Persistence & Reactive Item Resolution Suite (Modern 64px card architecture with Blizzard rarity borders and dismiss buttons, 24-hour auto-pruning, accurate "Needed By" crafter detection, standardized "1d 2h 34m" time display, localized client profession persistence fix, bulk crafting chat silence, work order spell collision fix, reactive uncached item preview loading, and tooltip crash prevention).
- **v1.3.4 (Released):** Inter-Addon Library Standardization & Ecosystem Compatibility (Replaced custom skeleton library mocks with canonical Ace3 and LibDeflate community releases, eliminating the alphabetical load-order collision that caused Questie v11.37.1 and other Ace3 addons to freeze silently on login; added `ChatThrottleLib` for disconnect-safe P2P syncing).
- **v1.3.5 (Released):** Guild Roster Interactive Sorting, Class Color Mapping & Recipe Drop Anti-Spam Suite (Clickable column headers with Blizzard sort arrows, unified online sorting, diacritic normalization for accented names, German female class color mappings, roster layout polish & golden border, scrollbar clearance fix, TradeSkillFrame button fix, 60s group drop announcement throttle, vendor buy suppression, and German "Alchimie" localization & DB migration).
